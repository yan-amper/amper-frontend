...
export const FiltersContainer = styled.aside`
  ...
  /**
   * Прокрутка внутри панели — только в низком окне, где фильтры реально
   * не помещаются. Раньше она стояла всегда, и на обычном экране панель
   * получала полосу прокрутки при полностью влезающем содержимом:
   * закрытые выпадашки фильтров — абсолютные блоки, они не занимают
   * места на экране, но область прокрутки собой раздвигают.
   * Заодно на нормальном экране перестала обрезаться раскрытая выпадашка
   * последнего фильтра.
   */
  ${media.shortViewport} {
